$tracking_number = preg_replace('/\s+/', '', $modx->getOption("track", $scriptProperties));
$tpl = $modx->getOption("tpl", $scriptProperties, "TrackingOrderView");
...
foreach ($tracking_urls as $item) {
	...
	if (count($match)) {
		$item['url'] = $item['url'] . preg_replace('/\s/', '', strtoupper($match[0]));
		$item['tracking_number'] = $tracking_number;
		
		return $modx->getChunk($tpl, $item);
	}
}
